...
def renderFilters():
    ...
    with RatingColumn:
        ...
        with col2:
            st.number_input(
                "Max rating",
                min_value=st.session_state.minRating,
                max_value=10.0,
                step=0.1,
                format="%.1f",
                key="maxRating",
...
def renderReportsTable(searchReports):
    rowsPerTable = 10
    ...
    start_index = (st.session_state.tableNumber - 1) * rowsPerTable
    end_index = min(st.session_state.tableNumber * rowsPerTable, len(searchReports))
...
def renderPageNumber(numOfTables):
    st.write("\n\n")
    maxPerRow = 10
...
#this css code was generated by ai (just to be clear)so I am able to make a divider in the color and shape I want :)
header_divider = """
<hr style="
    border: none;
    height: 2px;
